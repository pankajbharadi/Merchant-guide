...
/* Set of Roman numerals and their decimal values */
...
function CurrencyToValue(CurrencyArr) {
	...
	for (var ite = 0; ite < CurrencyArr.length; ite++) {
		...
		} else if (Metals[CurrencyArr[ite].toLowerCase()]) {
...
exports.Merchant = function(input) {
	...
	if (RegAns !== null) {
		...
		var value = CurrencyToValue(partials);
		if (value !== -1) {
			value *= Metals[unit.toLowerCase()];
			return console.log(RegAns[1].trim() + " is " + value.toFixed(0)
					+ " Credits");
		} else {
			return console.log("Invalid Currency");
		}
